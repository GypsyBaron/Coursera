...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		...
		// Vertices must have a value
		clause.push_back(i * 10);
		clause.push_back(i * 10 + 1);
		clause.push_back(i * 10 + 2);
		solver.addClause(clause);
		clause.clear();

		//Pair cant have same color
		clause.push_back(-1 * (i * 10));
		clause.push_back(-1 * (i * 10 + 1));
		solver.addClause(clause);
		clause.clear();

		clause.push_back(-1 * (i * 10));
		clause.push_back(-1 * (i * 10 + 2));
		solver.addClause(clause);
		clause.clear();
	...
	// Check if connected vertices have different color
	for (int i = 1; i <= m; i++) {

		cin >> a >> b;

		clause.push_back(-1 * (a * 10));
		clause.push_back(-1 * (b * 10));
		solver.addClause(clause);
		clause.clear();

		clause.push_back(-1 * (a * 10 + 1));
		clause.push_back(-1 * (b * 10 + 1));
		solver.addClause(clause);
		clause.clear();

		clause.push_back(-1 * (a * 10 + 2));
		clause.push_back(-1 * (b * 10 + 2));
		solver.addClause(clause);
		clause.clear();
